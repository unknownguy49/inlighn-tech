...
import { Shield, Code, Database, BarChart3, Clock, ArrowRight, CheckCircle, Star } from "lucide-react"
        ...
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
          <div className="text-center px-4 md:px-6 max-w-6xl mx-auto py-20">
            <ScrollReveal delay={0.2}>
              <Badge className="mb-6 md:mb-8 bg-blue-900/20 text-blue-300 border-blue-800 px-4 md:px-6 py-2 md:py-3 text-sm md:text-lg font-semibold backdrop-blur-sm">
                🎓 Immersive Programs
              </Badge>
            </ScrollReveal>
            ...
            <div className="text-4xl md:text-6xl lg:text-8xl font-black mb-6 md:mb-8 text-white leading-tight">
              ...
              <KineticText text=" Tomorrow's Technologies " className="inline" type="words" delay={400} />
            ...
            <ScrollReveal delay={0.6}>
              <p className="text-lg md:text-xl lg:text-2xl mb-8 md:mb-12 text-gray-300 max-w-4xl mx-auto leading-relaxed">
                Choose from our comprehensive internship programs designed to give you hands-on experience in the most
        ...
        <section className="py-16 md:py-24 lg:py-32 relative">
          <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-8 md:gap-12">
              {programsLoading
                ? Array.from({ length: 4 }).map((_, index) => <ProgramCardSkeleton key={index} />)
                : programs.map((program, index) => (
                    <ScrollReveal key={index} delay={index * 0.1}>
                      <Card className="bg-gray-900 border-gray-700 hover:shadow-2xl hover:border-blue-400 hover:shadow-blue-500/20 transition-all duration-300 group overflow-hidden h-full flex flex-col">
                        <div className="relative h-48 md:h-64 overflow-hidden">
                          ...
                          <div className="absolute top-4 right-4">
                            <Badge className="bg-white/90 text-gray-900 text-xs md:text-sm">{program.level}</Badge>
                          </div>
                          <div className="absolute bottom-4 left-4 flex items-center gap-2">
                            <Star className="h-3 w-3 md:h-4 md:w-4 text-yellow-400 fill-current" />
                            <span className="text-white font-medium text-sm">{program.rating}</span>
                            <span className="text-white/80 text-sm">({program.students})</span>
                          </div>
                        </div>
                        ...
                        <CardContent className="p-6 md:p-8 flex-1 flex flex-col">
                          <div className="flex items-center justify-between mb-4 md:mb-6">
                            ...
                            <div className="text-right">
                              <div className="text-2xl md:text-3xl font-black text-white">{program.price}</div>
                          ...
                          <h3 className="text-2xl md:text-3xl font-bold text-white mb-3 md:mb-4">{program.title}</h3>
                          <p className="text-gray-300 text-base md:text-lg leading-relaxed mb-4 md:mb-6 flex-1">
                            {program.description}
                          </p>

                          <div className="space-y-6 md:space-y-8">
                            <div>
                              <h4 className="text-white font-bold text-lg md:text-xl mb-3 md:mb-4 flex items-center gap-2">
                                <Star className="h-4 w-4 md:h-5 md:w-5 text-yellow-400" />
                                What You'll Master:
                              </h4>
                              <ul className="space-y-2 md:space-y-3">
                                {program.highlights.map((highlight, idx) => (
                                  <li key={idx} className="text-gray-300 flex items-center gap-3 text-sm md:text-base">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {program.tools.map((tool, idx) => (
                                  <Badge
                                    key={idx}
                                    variant="outline"
                                    className="border-gray-600 text-gray-300 hover:bg-gray-800 transition-colors text-xs md:text-sm"
                                  >
                                    {tool}
                                  </Badge>
                                ))}
                            ...
                            <Button
                              ...
                              className={`w-full bg-gradient-to-r ${program.color} hover:opacity-90 text-white font-bold py-3 md:py-4 text-base md:text-lg rounded-full transition-all duration-300 mt-auto`}
                            >
                              Apply Now <ArrowRight className="ml-2 h-4 w-4 md:h-5 md:w-5" />
                            </Button>
                          </div>
                        </CardContent>
                      </Card>
